						...
						synchronized (waitingLock) {
							...
							while (true) {
								...
								if (!someoneIsWorking) {
									...
									// are
									// alive.
									logger.info("It looks like no thread is working, waiting for "+ waitTime +" seconds to make sure...");
									sleep(waitTime);
									...
									if (!someoneIsWorking) {
										if (!shuttingDown) {
											...
											logger.info("No thread is working and no more URLs are in queue waiting for another "+ waitTime +" seconds to make sure...");
											sleep(waitTime);
										...
										logger.info("Waiting for "+ waitTime +" seconds before final clean up...");
										sleep(waitTime);
	...
	public void addSeed(String pageUrl, int docId) {
		...
		if (docId < 0) {
			...
			if (docId > 0) {
				...
				logger.info("Already seend URL: " + canonicalUrl);
				return;
			}
